...
export async function installFlushConflictPlugin() {
  ...
  console.log(
    chalk.blue(
      'eslint-config-prettier and eslint-plugin-prettier are installing...'
    )
  )
  ...
  child.stdout.on('end', () => {
    console.log(
      chalk.bgGreenBright(
        'eslint-config-prettier and eslint-plugin-prettier are installed'
      )
    )
    dealEslintConfig()
  })
...
function generateEslintConfig(oldConfig) {
  if (eslint_file === '.eslintrc.js') {
    if (!~oldConfig.indexOf('extends')) {
      ...
      return (
        oldConfig.substring(0, left_bracket + 1) +
        tempStr +
        oldConfig.substring(right_bracket)
      )
    }
  } else {
...
export async function runPrettier() {
  ...
  child.stdout.on('end', () => {
    console.log(
      chalk.greenBright(
        'Great!🌈 Auto formatted all files in the src directory'
      )
    )
  })
}
...
function removeFromGitignore() {
  readFile(path.join('.gitignore'), 'utf-8', (err, data) => {
    ...
    writeFile(path.join('.gitignore'), newData, (err) => {
      ...
      console.log(
        chalk.greenBright('Note: .vscode has been removed from .gitignore')
      )
    })
  })
}
